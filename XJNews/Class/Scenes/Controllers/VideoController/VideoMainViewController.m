...
@property (strong,nonatomic)VideoTableViewController *NVC;
@end
...
//模拟数据
- (void)simulateData{
    self.categories = [[NSArray alloc]initWithObjects:@"娱乐",@"精品",@"热点", nil];
}
...
- (void)addChildViewController{
    for(int i=0 ; i<self.categories.count; i++)
    {
        ...
        //循环创建子vc
        VideoTableViewController *mvc = [[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"videoTVC"];
        mvc.title = title;
        //传给下面的子vc标识，根据该标示做请求，获取数据
        
        mvc.index = i;
        switch (i) {
            case VideoEntertainment:mvc.urlStr = @"http://c.m.163.com/nc/video/list/V9LG4CHOR/n/0-25.html"; break;
            case VideoQuality:mvc.urlStr = @"http://c.m.163.com/nc/video/list/00850FRB/n/0-25.html"; break;
            case VideoHot:mvc.urlStr = @"http://c.m.163.com/nc/video/list/V9LG4B3A0/n/0-25.html"; break;
            default:
                break;
        }
...
- (void)addNavigateLable{
    for (int i = 0; i < self.categories.count; i++) {
        CGFloat lblW = 70;
        CGFloat lblX = i * lblW + [UIScreen mainScreen].bounds.size.width/5;
